# Analyse Détaillée de l'Implémentation de l'Algorithme FP-Growth
...
- `children`: LinkedList des enfants
- `node_link`: Lien vers le prochain nœud du même item (pour header table)

**Méthodes :**
- `find_child(item_name)`: **O(k)** où k = nombre d'enfants
...
**Complexités :**
- `insert(transaction)`: **O(m × k)** où m = longueur transaction, k = nombre enfants moyen
...
- O(n): Filtrer les items >= min_sup
- O(n²): Tri par insertion (`tri_insertion_desc`)
...
| `freq_pattern_set` | O(n²) | Tri par insertion |
...
| `FPTree.insert` | O(m × k) | Longueur trans × enfants |
...
2. **Children des FPNode:** Petit nombre d'enfants, parcours linéaire
...
## 6. **Architecture Globale: Flux de l'Algorithme**
...
6. CONDITIONAL_PATTERN_BASE ->Extraction des patterns
   
7. GENERATION DES RÈGLES (recursive mining)
```
...
Cette implémentation est **optimale** pour FP-Growth car:
...
- Utiliser un tri plus rapide (merge sort) pour `freq_pattern_set`: O(n log n)
